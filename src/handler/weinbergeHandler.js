...
const ev_ank = process.env.EV_ANKUENDIGUNG; // Get the Discord channel ID for Weinberge event announcements
const settingsPath = path.resolve(__dirname, '../settings.json');
const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8')); // Load settings from settings.json file

// Function to create the Weinberge event embed message for Discord
async function CreateFortyEmbed(guildName) {
    ...
    const baseEmbed = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`# Weinberge  ${timeKey}:15`), // Title
        )
        ...
        .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`- **Information:**\n> Um ${timeKey}:5 ausgerüstet an der Event-Zone`), // Info about event time
        )
        .addSeparatorComponents(
            new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true), // Divider
        ).addTextDisplayComponents(
...
// Main handler function to start the Weinberge event cron job
module.exports = function startHotelwarHandler(client) {
    // Schedule the handler to run at 20:05 every day
    cron.schedule("05 20 * * *", async () => {

        // Check if sending Weinberge event is enabled in the config table
        ...
        if (rows.length > 0 && rows[0].setconfig === 1) {
            ...
            // Log if sending Weinberge is not enabled
