...
const ev_ank = process.env.EV_ANKUENDIGUNG; // Get the Discord channel ID for hotel event announcements
const settingsPath = path.resolve(__dirname, '../settings.json');
const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8')); // Load settings from settings.json file

// Function to create the hotel event embed message for Discord
async function CreateFortyEmbed(guildName) {
    ...
    const baseEmbed = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`# Hotel  ${timeKey}:20`), // Title
        )
...
// Main handler function to start the hotel event cron job
module.exports = function startHotelwarHandler(client) {
    // Schedule the hotel handler to run at 02:10 every day
    cron.schedule("10 2 * * *", async () => {

        // Check if sending hotel event is enabled in the config table
        ...
        if (rows.length > 0 && rows[0].setconfig === 1) {
            try {
                ...
                console.error('❌ Error in hotel cron job:', err);
            }

        } else {
            // Log if sending hotel is not enabled
