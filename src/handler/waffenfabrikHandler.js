...
const ev_ank = process.env.EV_ANKUENDIGUNG; // Get the Discord channel ID for Waffenfabrik event announcements
const settingsPath = path.resolve(__dirname, '../settings.json');
const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8')); // Load settings from settings.json file

// Function to create the Waffenfabrik event embed message for Discord
async function CreateFortyEmbed(guildName) {
    ...
    const baseEmbed = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`# RPTicket Farbrik ${timeKey}:20`), // Title
        )
...
// Main handler function to start the Waffenfabrik event cron jobs
module.exports = function startWaffenfabrikHandler(client) {
    // Schedule the handler to run at 07:10, 10:10, and 22:10 every day
    ...
    // Function that sends the Waffenfabrik message if enabled in the config
    async function handleRPTicket(client) {
        // Check if sending Waffenfabrik is enabled in the config table
        ...
        if (rows.length > 0 && String(rows[0].setconfig) === '1') {
            ...
            // Log if sending Waffenfabrik is not enabled
