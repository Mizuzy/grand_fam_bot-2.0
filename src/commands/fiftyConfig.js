...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('fifty_config')
        .setDescription('Set map and/or priority for 50')
        ...
        .addStringOption(option =>
            option.setName('map')
                .setDescription('Name of the map (must exist for 50 in DB)')
                .setRequired(false)
    ...
    async execute(interaction) {
        ...
        try {
            ...
            // If a map is provided, check if it exists in the database for event '50'
            if (mapName) {
                const [mapRows] = await db.execute(
                    "SELECT ID FROM maps WHERE MAP = ? AND event = '50' LIMIT 1",
                    [mapName]
                );
                ...
                if (mapRows.length === 0) {
                    return await interaction.reply({
                        content: `❌ The map \`${mapName}\` does not exist or is not registered for 50.`,
                        ephemeral: true,
                    });
                }
            ...
            // Check if there is already an event entry for '50'
            const [rows] = await db.execute("SELECT ID, Prio, MapID FROM events WHERE Event = '50' LIMIT 1");
            ...
            if (rows.length > 0) {
                ...
                await db.execute(
                    "UPDATE events SET Prio = ?, MapID = ? WHERE Event = '50'",
                    [newPrio, newMapID]
                );
            } else {
                // If no entry exists, insert a new one
                await db.execute(
                    "INSERT INTO events (Event, Prio, MapID) VALUES ('50', ?, ?)",
